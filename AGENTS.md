...
**Core approach:** No custom ML training. Instead: bank transaction data → deterministic algorithms → Codex LLM synthesizes a final decision via `tool_use`.
...
The MVP uses **5 models** (not all 35 from the full spec). A single Codex agent orchestrates everything via `tool_use` — it decides which data to fetch and which algorithms to run, then synthesizes a final YES/NO answer.
...
| `agent/` | AI orchestration | Codex `tool_use` loop; decides which tools to call and in what order |
...
2. Codex calls MCP tools to fetch structured data from PostgreSQL
3. Codex calls algorithm tools with that data
4. Codex synthesizes all numeric results → YES/NO + Uzbek-language explanation
